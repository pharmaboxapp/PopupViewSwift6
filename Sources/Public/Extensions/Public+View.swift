...
public extension View {
    ...
    @MainActor func implementPopupView(config: (GlobalConfig) -> GlobalConfig = { $0 }) -> some View {
    #if os(iOS) || os(macOS)
        updateScreenSize()
...
public extension View {
    ...
    @MainActor func dismiss() { PopupManager.dismiss() }

    /// Dismisses all popups of provided type on the stack.
    @MainActor func dismiss<P: Popup>(_ popup: P.Type) { PopupManager.dismiss(popup) }

    /// Dismisses all popups on the stack up to the popup with the selected type
    @MainActor func dismissAll<P: Popup>(upTo popup: P.Type) { PopupManager.dismissAll(upTo: popup) }

    /// Dismisses all the popups on the stack.
    @MainActor func dismissAll() { PopupManager.dismissAll() }
}

// MARK: - Actions
//public extension View {
//    /// Triggers every time the popup is at the top of the stack
//    @MainActor func onFocus(_ popup: some Popup, perform action: @escaping () -> ()) -> some View {
